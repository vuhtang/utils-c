...
static struct region alloc_region(void const *addr, size_t query) {
  ...
  block_init(address, (block_size){.bytes = region_size}, NULL);
  return (struct region){
      .addr = address, .size = region_size, .extends = address == addr};
}
...
static inline bool block_is_suitable(struct block_header *block, size_t query) {
